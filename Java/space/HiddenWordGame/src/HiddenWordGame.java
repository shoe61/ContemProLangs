...
public class HiddenWordGame
{
	 ...
	 * 
	 * Handles all the input and output for both players. Displays prompts, hints, and messages
	 * and gets hidden word and guesses.
	 * 
	 * @param: Reason reason, enumerated; String hint, displayed to Player 2; int round, 
	...
	private static String getString(Reason reason, String hint, int round)
	{
		...
		{
			// Print congratulations and a comment on the player's performance.
			
			System.out.println("Congratulations! You won the game in " + (round-1) + " rounds!");
						
			if(lastRound == 1)
			{
				// NOTE: since the unicode escape characters \u0022 for quotation marks within a
				// string are in separate strings, the additional escape \u005c is required. 
				// Thanks, Stack Overflow!
				
				System.out.println("Guessing a " + hint.length() + " letter word like \u005c\u0022"
						+ hint + "\u005c\u0022 in one try is possible but unlikely; \nif it " +
						"happens too frequently" + " the Board of Governors will order an " + 
						"investigation.");
			}
			
			else if(lastRound == 2)
			{
				System.out.println("You guessed the hidden word \u005c\u0022" + hint + "\u005c\u0022 in" +
						" just two guesses... \nyou might consider trying to make a living at" +
						" this game.");
			}
