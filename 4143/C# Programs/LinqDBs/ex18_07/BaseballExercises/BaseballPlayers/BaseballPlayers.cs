...
namespace BaseballPlayers
{
   public partial class BaseballPlayers : Form
   {
         ...
         object sender, EventArgs e )
      {
         ...
         RefreshPlayers(); // change back to initial unfiltered data 
      } // end method playerBindingNavigatorSaveItem_Click   

      // filter to display only players with
      // batting averages in the specified range
      private void averageButton_Click( object sender, EventArgs e )
      {
         // convert strings in TextBoxes to decimals
         decimal minimum = Convert.ToDecimal( minimumTextBox.Text );
         decimal maximum = Convert.ToDecimal( maximumTextBox.Text );

         // use LINQ to select only players in the range given in
         // minimumTextBox and maximumTextBox
         var averageQuery =
         ...
         bindingNavigatorAddNewItem.Enabled = false;
         bindingNavigatorDeleteItem.Enabled = false;
      } // end method averageButton_Click
